...
function renderBalancedPlate(entries){
  ...
  if(totalW<=0){
    ...
    ['veg','prot','starch','fat'].forEach(function(k){
      ...
      document.getElementById('plate-'+k+'-target').style.left=emptyTgt[k]+'%';
    });
    return;
  }
  ...
  if(capEl) capEl.innerHTML = 'La ligne <b>\u2503</b> sur chaque barre indique le rep\u00e8re pour votre niveau d\'activit\u00e9 ('+(actLabels[actKey]||'s\u00e9dentaire')+') \u2014 pas besoin d\'atteindre pile 100% au total, chaque famille compte ind\u00e9pendamment.';

  document.getElementById('plate-veg-fill').style.width=vegPct+'%';
  document.getElementById('plate-veg-pct').textContent=vegPct+'%';
  document.getElementById('plate-veg-target').style.left=plateTgt.veg+'%';
  document.getElementById('plate-prot-fill').style.width=protPct+'%';
  document.getElementById('plate-prot-pct').textContent=protPct+'%';
  document.getElementById('plate-prot-target').style.left=plateTgt.prot+'%';
  ...
  document.getElementById('plate-starch-target').style.left=plateTgt.starch+'%';
  document.getElementById('plate-fat-fill').style.width=fatPct+'%';
  document.getElementById('plate-fat-pct').textContent=fatPct+'%';
  document.getElementById('plate-fat-target').style.left=plateTgt.fat+'%';
