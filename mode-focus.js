...
function renderFocusGuidance(T, tgt){
  ...
  // DIAGNOSTIC TEMPORAIRE -- admin uniquement, a retirer une fois le bug trouve.
  if(PROF && PROF.role==='admin' && typeof toast==='function'){
    toast('\ud83d\udd0d focusGuidance: el='+(!!el)+' T='+(T?'ok':'manquant')+' tgt.kcal='+(tgt?tgt.kcal:'manquant')+' satiety='+SATIETY_TODAY+' role='+(PROF&&PROF.role));
  }
  if(!el) return;
  ...
  var msg;
  if(ratio < 0.5){
    msg = { ico:'\ud83c\udf31', txt:'Vous avez encore bien de la place aujourd\'hui \u2014 mangez selon votre faim, sans compter.' };
  } else if(ratio < 0.85){
    msg = { ico:'\ud83d\udc4d', txt:'Vous avancez bien dans votre journ\u00e9e \u2014 continuez \u00e0 \u00e9couter votre faim, repas apr\u00e8s repas.' };
  } else if(ratio < 1.15){
    msg = { ico:'\u2696\ufe0f', txt:'Vous \u00eates dans la zone qui vous correspond aujourd\'hui. Encore faim ? Continuez sans souci. Rassasi\u00e9(e) ? C\'est peut-\u00eatre le bon moment de vous arr\u00eater.' };
  } else {
    msg = { ico:'\ud83c\udf3f', txt:'Vous avez d\u00e9j\u00e0 bien mang\u00e9 aujourd\'hui. Si vous avez encore vraiment faim, \u00e9coutez-la \u2014 sinon, ce sera peut-\u00eatre le moment de vous arr\u00eater pour aujourd\'hui.' };
  }
...
var STARCH_REGEX = /pates|penne|fusilli|spaghetti|macaroni|tagliatelle|linguine|coquillette|farfalle|lasagne|nouilles|rice\b|riz|pain|pomme de terre|patate|quinoa|semoule|boulgour|cereales?|muesli|granola|avoine|flocons|biscotte|tortilla|couscous|polenta|orge|sarrasin/;
...
function renderBalancedPlate(entries){
  ...
  // Classification bas\u00e9e sur le macronutriment R\u00c9ELLEMENT DOMINANT de
  // chaque aliment (glucides/prot\u00e9ines/lipides en grammes, d\u00e9j\u00e0 connus
  // pour chaque entr\u00e9e du journal) plut\u00f4t qu'une liste de mots-cl\u00e9s \u00e0
  // rallonge qu'il fallait sans cesse compl\u00e9ter (pennes, muesli, riz en
  // anglais...). Principe : on regarde ce qui pousse le plus dans
  // l'aliment, avec un seul cas particulier \u2014 si ce sont des glucides qui
  // dominent, on v\u00e9rifie d'abord si c'est un fruit/l\u00e9gume/l\u00e9gumineuse
  // (auquel cas \u00e7a va dans "L\u00e9gumes & fruits") avant de le ranger par
  // d\u00e9faut dans "F\u00e9culents". \u00c7a couvre automatiquement N'IMPORTE QUEL
  // aliment de la base, m\u00eame un dont le nom n'est dans aucune liste.
  var vegW=0, protW=0, starchW=0, fatW=0;
  ...
  // 100% du poids mang\u00e9 \u00e0 cet instant, donc affichait 100% \u2014 trompeur, \u00e7a
  ...
  var vegBarPct = gramBarPct(vegW, gramTgt.veg);
  var protBarPct = gramBarPct(protW, gramTgt.prot);
  var starchBarPct = gramBarPct(starchW, gramTgt.starch);
  var fatBarPct = gramBarPct(fatW, gramTgt.fat);

  document.getElementById('plate-veg-fill').style.width=vegBarPct+'%';
  document.getElementById('plate-veg-pct').textContent=vegBarPct+'%';
  document.getElementById('plate-prot-fill').style.width=protBarPct+'%';
  document.getElementById('plate-prot-pct').textContent=protBarPct+'%';
  document.getElementById('plate-starch-fill').style.width=starchBarPct+'%';
  document.getElementById('plate-starch-pct').textContent=starchBarPct+'%';
  document.getElementById('plate-fat-fill').style.width=fatBarPct+'%';
  document.getElementById('plate-fat-pct').textContent=fatBarPct+'%';
  ...
  // Le vrai % proportionnel (utilis\u00e9 seulement pour le message de
  // structure ci-dessous, jamais pour l'affichage des barres) \u2014 avec un
  // seuil minimum de poids avant de se prononcer, pour ne pas donner un
  // verdict "tr\u00e8s \u00e9quilibr\u00e9e" pr\u00e9matur\u00e9 sur un seul aliment.
  ...
  if(totalW < 250){
    ...
    // (une banane par ex.) peut repr\u00e9senter 100% de ce qui a \u00e9t\u00e9 mang\u00e9
