...
function renderBalancedPlate(entries){
  ...
  // Arrondi INDÉPENDANT plutôt que "ce qui reste après les 3 autres" —
  // sinon une petite quantité réelle de matières grasses (une cuillère
  // d'huile par ex.) peut disparaître complètement à cause de l'arrondi
  // cumulé des 3 autres catégories (ex: 46+26+28=100 alors que la vraie
  // somme non arrondie ne fait que 98,7% à cause des décimales).
  ...
  if(capEl) capEl.innerHTML = 'Chaque barre montre votre progression vers le rep\u00e8re de votre niveau d\'activit\u00e9 ('+(actLabels[actKey]||'s\u00e9dentaire')+') \u2014 100% = objectif atteint pour cette famille.';

  // Chaque barre affiche sa PROGRESSION vers sa propre cible (plafonn\u00e9e \u00e0
  // 100%), plut\u00f4t que le pourcentage r\u00e9el du poids total \u2014 plus simple \u00e0
  // lire d'un coup d'\u0153il : "est-ce que j'ai atteint mon repère pour cette
  // famille ?" plutôt que d'avoir à comparer deux chiffres entre eux. Le
  // vrai pourcentage (vegPct etc.) reste utilis\u00e9 en interne pour le
  // message d'\u00e9quilibre global, plus bas.
  function barPct(actualPct, targetPct){
    if(targetPct<=0) return 0;
    return Math.min(100, Math.round(actualPct/targetPct*100));
  }
  var vegBarPct = barPct(vegPct, plateTgt.veg);
  var protBarPct = barPct(protPct, plateTgt.prot);
  var starchBarPct = barPct(starchPct, plateTgt.starch);
  var fatBarPct = barPct(fatPct, plateTgt.fat);
