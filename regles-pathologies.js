...
function detectFoodCategory(nom){
  ...
  if(/pain blanc|baguette|pain de mie/.test(n)) return 'pain_blanc';
  if(/riz blanc/.test(n)) return 'riz_blanc';
  if(/pates cuites/.test(n) && !/complet/.test(n)) return 'pates_blanches';
  if(/corn flakes|chocapic|frosties|miel pops|cereale/.test(n)) return 'cereales_sucrees';
  // Le chocolat blanc et au lait sont détectés même sans valeurs nutritionnelles
  // renseignées — le nom seul suffit à savoir que c'est à surveiller.
  if(/chocolat blanc|chocolat au lait|tablette.*chocolat|chocolat.*blanc|chocolat.*lait/.test(n)) return 'chocolat';
  return null;
}
