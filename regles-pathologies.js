...
// Rendu UI ultra-\u00e9pur\u00e9 (section 3 de la demande) \u2014 un badge, et UNE seule
// ligne de suggestion dans le seul cas o\u00f9 elle apporte une action concr\u00e8te
// (absorption rapide ou boisson sucr\u00e9e d\u00e9tect\u00e9e). Aucune explication
// th\u00e9orique affich\u00e9e par d\u00e9faut \u2014 l'intelligence reste dans le code.
function glycemicMealBadgeHTML(mealItems){
  var pathos = (typeof PROF!=='undefined' && PROF && PROF.pathologies) || [];
  if(pathos.indexOf('diabetes')<0) return '';
  if(!mealItems || !mealItems.length) return '';
  var res = evaluateGlycemicImpact(mealItems);
  if(res.state==='LOW_CARB') return ''; // impact glyc\u00e9mique minime, pas besoin d'un badge

  var meta = {
    OPTIMAL_BUFFER:  {ico:'\ud83d\udfe2', lbl:'Glyc\u00e9mie stabilis\u00e9e',        cls:'meal-glyc-ok'},
    MODERATE_BUFFER: {ico:'\ud83d\udfe0', lbl:'Tampon glyc\u00e9mique mod\u00e9r\u00e9',  cls:'meal-glyc-mid'},
    FAST_ABSORPTION: {ico:'\ud83d\udfe1', lbl:'Glucides \u00e0 absorption rapide', cls:'meal-glyc-warn'}
  }[res.state];

  var html = '<div class="meal-glyc-badge '+meta.cls+'">'+meta.ico+' '+meta.lbl+'</div>';
  if(res.state==='FAST_ABSORPTION' || res.hasSweetLiquid){
    var tip = res.liquidWarning || 'Associez ces glucides \u00e0 des fibres ou des prot\u00e9ines au prochain repas.';
    html += '<div class="meal-glyc-tip">'+tip+'</div>';
  }
  return html;
}
